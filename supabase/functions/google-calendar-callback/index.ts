...
Deno.serve(async (req) => {
  ...
  const fail = (appUrl = Deno.env.get("APP_URL") || "http://127.0.0.1:8000/", detail = "connect_failed") =>
    Response.redirect(withParam(appUrl, "google_calendar", detail), 302);
  ...
  const { error: saveError } = await admin.from("google_oauth_tokens").upsert({
    user_id: parsed.uid,
    provider: "google",
  ...
  return Response.redirect(withParam(parsed.appUrl, "google_calendar", "connected"), 302);
});
