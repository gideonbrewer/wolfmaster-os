...
Deno.serve(async (req) => {
  ...
  const fail = (appUrl = Deno.env.get("APP_URL") || "http://127.0.0.1:8000/", detail = "connect_failed") =>
    Response.redirect(withParam(appUrl, "google_gmail", detail), 302);
  ...
  const { error: saveError } = await admin.from("google_gmail_tokens").upsert({
    user_id: parsed.uid,
    provider: "gmail",
    email,
  ...
  return Response.redirect(withParam(parsed.appUrl, "google_gmail", "connected"), 302);
});

function gmailRedirectUri() {
  return Deno.env.get("GOOGLE_GMAIL_REDIRECT_URI") ||
    `${Deno.env.get("SUPABASE_URL")}/functions/v1/google-gmail-callback`;
}
